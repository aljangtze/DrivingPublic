...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
                            <form action="{{ url('admin/lessiontwo') }}" method="get">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
				<li>
					<div class="input-group" >
					  <input type="text" name="name" class="form-control" placeholder="输入关键词搜索" style="width:300px;" />
	...
	<div class="glzh">
		...
		<a href="{{ url('admin/lessiontwoysh') }}"><button type="button" class="btn btn-info">已审核</button></a>
	</div>
	<!--表格信息-->
        <input type="hidden" id="url" value="{{ url('admin/lessiontwosqdel') }}">
			...
			<tr>
				...
				<td><input style="width: 40px;height:30px" id="px{{ $tmp->id }}" onblur="dopxx({{ $tmp->id }})" type="tex" name="px" value="{{ $tmp->px }}"></td>
                                ...
                                <td>
                                    ...
                                    @if($tmp->lxmodel == 2) 考试练习 @endif
                                </td>
                                <td>{{ $tmp->fbdate }}</td>
                                <td>{{ $tmp->lcaddress }}</td>
                                <td>@if($tmp->sh == 2) <font color="red">未审核</font> @endif</td>
				<td>
					<a href='{{ url("admin/lessiontwo/$tmp->id/edit") }}' class="btn btn-info">修改</a>
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger">删除</a>
					<a href="{{ url("admin/lessiontwosh")."/".$tmp->id }}" class="btn btn-info">审核</a>
				</td>
			</tr>
                        @endforeach
		</table>
    ...
    <form action="{{ url('admin/lessiontwo') }}" name="myform" method="post" style="display: none">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="_method" value="delete">
    </form>
    <script>
        function dodel(id){
            dodell(id,"{{ url('admin/lessiontwo') }}");
        }
        function dopxx(id){
            var px = $("#px"+id+"").val();
            dopx(px,"{{ url('admin/lessiontwopx') }}",id);
        }
    </script>
<script>
//复选框事件
