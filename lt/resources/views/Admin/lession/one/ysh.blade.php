...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
                            <form action="{{ url('admin/lessionone') }}" method="get">
	...
	<div class="glzh">
		...
		<a href="{{ url('admin/lessionone/create') }}"><button type="button" class="btn btn-info">已审核</button></a>
	</div>
	<!--表格信息-->
        <input type="hidden" id="url" value="{{ url('admin/lsessionsqdel') }}">
	<div class="table_yw" style="width:98%;margin:0 auto;">
		<table class="table table-bordered">
			...
			<tr>
				...
				<td>{{ $tmp->lxaddress }}</td>
                                <td>
                                    @if($tmp->sh == 1) <font color="blue">已审核</font> @endif
                                </td>
				<td>
					<a href='{{ url("admin/lessionone/$tmp->id/edit") }}' class="btn btn-info">修改</a>
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger">删除</a>
					<!--<a href='{{ url("admin/lessiononesh/$tmp->id") }}' class="btn btn-info">审核</a>-->
				</td>
			</tr>
                        @endforeach
		</table>
    ...
    <form action="{{ url('admin/lessionone') }}" name="myform" method="post" style="display: none">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="_method" value="delete">
    </form>
    <script>
        function dodel(id) {
            dodell(id,"{{ url('admin/lessionone') }}");
        }
        
        function dopxx(id){
            var px = $("#px"+id+"").val();
            dopx(px,"{{ url('admin/lessiononepx') }}",id);
        }
    </script>
<script>
//复选框事件
