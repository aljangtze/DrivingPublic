...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
				<li>
                                    <form action="{{ url('admin/tjnormal') }}" method="get">
	...
	<div class="glzh">
		<button type="button" class="btn btn-danger" id="sqdel">全部删除</button>
                <input type="hidden" id="url" value="{{ url('admin/ptclasssqdel') }}">
                <a href="{{ url('admin/ptysh') }}"><button type="button" class="btn btn-info">已审核</button></a>
                <a href="{{ url("admin/ptclass") }}"><button type="button" class="btn btn-danger">未审核</button></a>
...
<!--                                    @if($tmp->sh == 1)
				...
				<td>
					<a href='{{ url("admin/ptclass/$tmp->id/edit") }}' class="btn btn-info">修改</a>
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger">删除</a>
                                        <a href="{{ url('admin/ptclasssh')."/".$tmp->id }}" class="btn btn-info">审核</a>
				</td>
			</tr>
                        @endforeach
		</table>
    ...
    <script>
        function dodel(id) {
            dodell(id,"{{ url('admin/ptclass') }}");
        }
        
        function dopxx(id){
            var px = $("#px"+id+"").val();
            dopx(px,"{{ url('admin/ptclasspx') }}",id);
        }
    </script>
<script>
//复选框事件
