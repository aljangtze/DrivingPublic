...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
				<li>
                                    <form action="{{ url('admin/zxjlysh') }}" method="get">
					<div class="input-group" >
					  <input type="text" class="form-control" name="name" placeholder="输入关键词搜索" style="width:300px;" />
	...
	<div class="tb_operation">
		...
		<a href="{{ url('admin/zxjl') }}"><button type="button" class="btn btn-success">（{{ $count }}）未审核</button></a>
	</div>
        <input type="hidden" id="url" value="{{ url('admin/zxjlsqdl') }}">
	<!--表格信息-->
	<div class="table_yw">
		<table class="table table-bordered">
			...
			<tr>
				...
				<td>{{ $tmp->sfzh }}</td>
                                ...
                                <td><font color="blue">@if($tmp->sh == 1) 已审核 @endif</font></td>
                                <td>
                                    <font color="blue">@if($tmp->sha == 1) 已通过 @endif</font>
                                    <font color="red">@if($tmp->sha == 2) 未通过 @endif</font>
                                </td>
				<td>
					<a href="{{ url('admin/zxjledit')."/".$tmp->id."/".$tmp->type }}" class="btn btn-info">修改</a>
    ...
    <form action="{{ url('admin/zxjl') }}" method="post" name="myform" style="display: none">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="_method" value="delete">
    </form>
    <script>
        function dodel(id) {
            dodell(id,"{{ url('admin/zxjl') }}");
        }
