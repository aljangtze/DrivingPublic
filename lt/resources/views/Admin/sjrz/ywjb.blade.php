...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" style="display:none;" id="sscs">
			<ul>
                            <form action="{{ url('admin/jpfw') }}" method="get">
				...
				<li>
					<div class="form-group">
						<label class="">驾校</label>
						<input type="text" name="jxname" class="form-control" placeholder="请输入所在驾校" />
					</div>
				</li>
<!--				<li>
					<div class="form-group">
	...
	<div class="table_yw">
		<table class="table table-bordered">
			<tr class="info">
				...
				<td>驾校位置</td>
				<td width="4%">营业执照</td>
                                <td width="4%">法人身份证正面</td>
				<td width="4%">法人身份证反面</td>
				<td width="4%">驾校门头照</td>
				<td width="4%">训练场地一</td>
				<td width="4%">训练场地二</td>
				<td width="4%">审核状态</td>
			...
			<tr>
				...
				<td>{{ $tmp->id }}</td>
                                ...
                                <td>{{ $tmp->jxaddress }}</td>
                                <td>
                                    <a href="{{ asset('sjrz')."/".$tmp->yyzzpic }}"><image width="70px" height="40px" src="{{ asset('sjrz/s_').$tmp->yyzzpic }}"></a>
                                </td>
                                <td>
                                    <a href="{{ asset('frsfzpicz')."/".$tmp->frsfzpicz }}"><image width="70px" height="40px" src="{{ asset('frsfzpicz/s_').$tmp->frsfzpicz }}"></a>
                                </td>
                                <td>
                                    <a href="{{ asset('frsfzpicf')."/".$tmp->frsfzpicf }}"><image width="70px" height="40px" src="{{ asset('frsfzpicf/s_').$tmp->frsfzpicf }}"></a>
                                </td>
                                <td>
                                    <a href="{{ asset('jlcpic')."/".$tmp->jlcpic }}"><image width="70px" height="40px" src="{{ asset('jlcpic/s_').$tmp->jlcpic }}"></a>
                                </td>
                                <td>
                                    <a href="{{ asset('xlcdpic1')."/".$tmp->xlcdpic1 }}"><image width="70px" height="40px" src="{{ asset('xlcdpic1/s_').$tmp->xlcdpic1 }}"></a>
                                </td>
                                <td>
                                    <a href="{{ asset('xlcdpic2')."/".$tmp->xlcdpic2 }}"><image width="70px" height="40px" src="{{ asset('xlcdpic2/s_').$tmp->xlcdpic2 }}"></a>
                                </td>
                                <td>
                                    @if($tmp->sh == 1)
				...
				<td>
                                    <a href="{{ url('admin/jpform')."/".$tmp->id }}" class="btn btn-primary"><font color="white">设置</font></a>&nbsp;&nbsp;
                                    <a href="{{ url('admin/jpedit')."/".$tmp->id }}" class="btn btn-success"><font color="white">修改</font></a>&nbsp;
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger"><font color="white">删除</font></a>&nbsp;&nbsp;
                                        @if($tmp->sh == 2)
					<a href="{{ url('admin/sh')."/".$tmp->id }}" class="btn btn-info"><font color="white">审核</font></a>
                                        @endif
                                        @if($tmp->sh == 1)
					<a href="javascript:alert('不能重复审核')" class="btn btn-info"><font color="white">审核</font></a>
                                        @endif<br />
					@if($tmp->sh == 1)
                                        <a data-toggle="modal" data-target="#myModal{{ $tmp->id }}" class="btn btn-warning"><font color="black">班型发布</font></a>&nbsp;&nbsp;
                                        @endif
                                        @if($tmp->sh == 2)
                                        <a href="javascript:alert('审核未通过，请先审核再来发布')" class="btn btn-default"><font color="black">班型发布</font></a>&nbsp;&nbsp;
                                        @endif
                                        <a href="{{ url('admin/zzfbclassindex')."/".$tmp->id }}" class="btn btn-default"><font color="black">查看班型</font></a>
				</td>
