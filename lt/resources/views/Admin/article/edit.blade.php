...
<body>
    ...
    <ul class="forminfo">
        <form action="{{ url('admin/article')."/".$str->id }}" method="post" enctype="multipart/form-data">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="_method" value="PUT">
            <input type="hidden" name="mypic" value="{{ $str->pic }}">
        <li><label>文章标题</label><input name="title" type="text" class="dfinput" value="{{ $str->title }}"/><i></i></li>
        <li><label>图片</label><input name="pic" type="file" class="dfinput" /><i>*必选</i></li>
        <li><label>投放位置</label><cite><input name="type" @if($str->type == 1) checked @endif type="radio" value="1"/>关于我们</li>
        <li><label>文章内容</label><textarea id="content" style="width:1000px;height:800px" name="content" style="margin-left:85px;">{!!$str->content!!}</textarea></li>
