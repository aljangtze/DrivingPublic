...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
                            <form action="{{ url('admin/lzqcfw') }}" method="get">
	...
	<!--表格操作-->
        <input type="hidden" id="url" value="{{ url('admin/lzqcfwsqdel') }}">
	<div class="glzh">
		<button type="button" class="btn btn-danger" id="sqdel">全部删除</button>
		<a href="{{ url('admin/lzqcfw/create') }}"><button type="button" class="btn btn-info">已审核</button></a>
	</div>
	<!--表格信息-->
	<div class="table_yw" style="width:98%;margin:0 auto;">
		<table class="table table-bordered">
			...
			<tr>
				...
				<td>
					<a href='{{ url("admin/lzqcfw/$tmp->id/edit") }}' class="btn btn-info">修改</a>
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger">删除</a>
					<a href='{{ url("admin/lzqcfwsh/$tmp->id") }}' class="btn btn-info">审核</a>
				</td>
			</tr>
                        @endforeach
		</table>
    ...
    <form action="{{ url('admin/lzqcfw') }}" name="myform" method="post" style="display: none">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">   
    <input type="hidden" name="_method" value="delete">   
    </form>
    <script>
        function dodel(id) {
            dodell(id,"{{ url('admin/lzqcfw') }}");
        }
        
        function dopxx(id){
            var px = $("#px"+id+"").val();
            dopx(px,"{{ url('admin/lzqcfwpx') }}",id);
        }
    </script>
<script>
//复选框事件
