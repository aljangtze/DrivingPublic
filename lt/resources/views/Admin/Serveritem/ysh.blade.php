...
<div class="info_box">
	...
	<form class="form-inline form_ss">
		...
		<div class="search_nr" id="sscs">
			<ul>
                            <form action="{{ url('admin/lzjrzfw/create') }}" method="get">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
				<li>
					<div class="input-group" >
					  <input type="text" name="name" class="form-control" placeholder="输入姓名" style="width:300px;" />
	...
	<!--表格操作-->
        <input type="hidden" id="url" value="{{ url('admin/lzjrzfwsqdel') }}">
	<div class="glzh">
		<button type="button" class="btn btn-danger" id="sqdel">全部删除</button>
		<a href="{{ url('admin/lzjrzfw/create') }}"><button type="button" class="btn btn-info">已审核</button></a>
	</div>
	<!--表格信息-->
	<div class="table_yw" style="width:98%;margin:0 auto;">
		<table class="table table-bordered">
			...
			<tr>
				...
				<td>
					<a href='{{ url("admin/lzjrzfw/$tmp->id/edit") }}' class="btn btn-info">修改</a>
					<a href="javascript:dodel({{ $tmp->id }})" class="btn btn-danger">删除</a>
					<!--<a href='{{ url("admin/lzjrzfwsh/$tmp->id") }}' class="btn btn-info">审核</a>-->
				</td>
			</tr>
                        @endforeach
		</table>
    ...
    <form action="{{ url('admin/lzjrzfw') }}" name="myform" method="post" style="display: none">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">   
    <input type="hidden" name="_method" value="delete">   
    </form>
    <script>
        function dodel(id) {
            dodell(id,"{{ url('admin/lzjrzfw') }}");
        }
        
        function dopxx(id){
            var px = $("#px"+id+"").val();
            dopx(px,"{{ url('admin/lzjrzfwpx') }}",id);
        }
    </script>
<script>
//复选框事件
