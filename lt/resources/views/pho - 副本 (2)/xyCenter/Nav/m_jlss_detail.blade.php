...
<html>
    ...
    <header class="header" style=" margin-bottom:0;">
        ...
        <form action="{{ url('tel/seljl')."/".session()->get("Teluser") }}" class="form-inline">
                ...
                <div id="distpicker">
                    <div class="form-group" style="width:80%;float:left;margin-right:1%;">
                        <div style="position: relative;">
                            <input id="city-picker3" name="address" class="form-control" readonly type="text" value="请选择省/市/区县" data-toggle="city-picker">
                        </div>
                    </div>
            ...
            <dl class="topmenu">
                ...
                <!--			<dt style="width:50%;">
                                                <div class="selectlist">
                                                        <div class="select_textdiv">
                                                                <input type="hidden" value=""/>
                                                                <p class="s_text">性别</p><span class="down"><img src="{{ asset('phone/images/down.png') }}"></span>
                                                        </div>
                                                        <div class="select_textul">
                                                                <ul class="select_first_ul">
                                                                        <li>
    ...
    <div class="coach_box">
        <ul>
            ...
            @if($v->type == 1)
            <li>
                <a href="{{ url('tel/jldetail')."/".$v->id."/".$v->id."/".session()->get("Teluser") }}">
                    <div class="coach_img">
                        <img src="{{ asset('jlcpic')."/".$v->jlcpic }}" />
                    </div>
                </a>
                <div class="coach_xx">
                    <p><a href="{{ url('tel/jldetail')."/".$v->id."/".$v->id."/".session()->get("Teluser") }}">{{ $v->name }}</a></p>
                    <p style="font-size:1.5rem;color:#f00;font-weight:bold">￥{{ $v->jpprice }}元</p>
                    <p>报名数：{{ $count[$v->id] }}人</p>
                    <p class="dhyc_p" style="height:40px;"><span>{{ $v->jxaddress }}</span></p>
                </div>
            </li>
            @endif
<!--            @if($v->type == 2)
            <li>
                <a href="{{ url('tel/jldetail')."/".$v->id."/".$v->id."/".session()->get("Teluser") }}">
                    <div class="coach_img">
                        <img src="{{ asset('dppic')."/".$v->dppic }}" />
                    </div>
                </a>
                <div class="coach_xx">
                    <p><a href="{{ url('tel/jldetail')."/".$v->id."/".$v->id."/".session()->get("Teluser") }}">{{ $v->name }}</a></p>
                    <p style="font-size:1.5rem;color:#f00;font-weight:bold">￥40元</p>
                    <p>成交量：40</p>
