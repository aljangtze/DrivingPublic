...
<header class="header">
	...
	<p style="width:84%;text-align:center;color:#fff;height:40px; line-height:40px;">科目四发布</p>
</header>
<!--汽车服务内容-->
<div class="vip_class">
	<div class="jm1">
            <form action="{{ url('tel/savelessionfour')."/".session()->get("Teluser") }}" method="post">
			...
			<div class="fenshu">
				<label>所学车型</label>
				<input type="text" value="" name="cartype" placeholder="请填写所学车型" id=""/>
			</div>
			<div class="fenshu">
			...
			<!--时间段-->
			<div class="xm_check">
				<p style="color:#333;">时间选择(可多选)</p>
				<label>
					<input name="seltime1" type="checkbox" value="8:30-12:30"/> 8:30-12:30
				</label>
				<label>
					<input name="seltime2" type="checkbox" value="13:30-17:30"/> 13:30-17:30
				</label>
				<label>
					<input name="seltime3" type="checkbox" value="18:30-21:30"/> 18:30-21:30
				</label>
				<label>
					<input name="seltime4" type="checkbox" value="全天"/> 全天
				</label>
			</div>
			<p class="clearfix" style="height:0;margin-bottom:0;"></p>
			<div class="fenshu">
				<label>联系电话</label>
				<input type="text" value="" name="lxtel" placeholder="请填写联系电话，必填" id="phone" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onblur="this.v();" />
			</div>
			<div class="fenshu">
				<label>练习地址</label>
				<input type="text" value="" name="lxaddress" placeholder="请填写练习地址，必填" id="addr"/>
			</div>
			<!--上传附件-->
			<!--<div class="image_up">
...
$("#btn_ptbx").click(function(){
	...
	else if(price=="" || appDate=="" || phone=="" || addr==""){
		alert("内容不能为空");
		return false;
	}
	else{
